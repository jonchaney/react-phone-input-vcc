...
class PhoneInput extends React.Component {
  ...
  handleFlagDropdownClick = () => {
    ...
    if (preferredCountries.includes(selectedCountry)) {
      highlightCountryIndex = preferredCountries.findIndex(
        (o) => o == selectedCountry
      );
    } else {
      highlightCountryIndex = this.props.enableAreaCodes
        ? allCountries.findIndex((o) => o == selectedCountry)
    ...
    this.setState(
      ...
      () => {
        if (
          ...
          if (
            formattedNumber.length > caretPosition - 1 &&
            formattedNumber.charAt(caretPosition - 1) === ")"
          ) {
            caretPosition += 2;
          }
          if (
            formattedNumber.length > caretPosition - 1 &&
            formattedNumber.charAt(caretPosition) === "("
          ) {
            caretPosition += 1;
          }
          if (
            formattedNumber.length >= caretPosition &&
            !formattedNumber.charAt(caretPosition - 1).match(/^[0-9]+$/)
          ) {
            caretPosition += 1;
          }
  ...
  getSearchFilteredCountries = () => {
    ...
    if (enableSearch && sanitizedSearchValue) {
      ...
      const iso2countries = allCountries.filter(
        ({ name, localName, iso2, dialCode }) =>
          [`${iso2}`].some((field) =>
      ...
      const searchedCountries = allCountries.filter(
        ({ name, localName, iso2, dialCode }) =>
          [
            `${name}`,
            `${localName}`,
            this.props.prefix + dialCode,
          ].some((field) => field.toLowerCase().includes(sanitizedSearchValue))
      );
      this.scrollToTop();
